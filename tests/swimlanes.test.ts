...
describe("filterLaneEntries", () => {
	...
	it("keeps only the entries the predicate accepts", () => {
		...
		const filtered = filterLaneEntries(lanes(), (e) =>
			kept.has((e as unknown as { name: string }).name),
		);
	...
	it("keeps every lane and column even when everything is filtered out", () => {
		const filtered = filterLaneEntries(lanes(), () => false);
	...
	it("does not mutate the input", () => {
		...
		filterLaneEntries(original, () => false);
